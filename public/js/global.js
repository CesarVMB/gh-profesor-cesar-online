...
document.querySelectorAll(".dropdown-menu > li > a").forEach(link=>{if(link.href===currentUrl){link.classList.add("active");link.setAttribute("aria-current","page")}if(currentCategory&&link.href.includes(`/categoria/${currentCategory}`)){link.classList.add("active");link.setAttribute("aria-current","page")}});// SHOW THE CURRENT YEAR
